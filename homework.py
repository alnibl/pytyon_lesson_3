...
# 3) Перевести все слова к нижнему регистру (map).

new_text_list = list(map(lambda x: x.lower(), text_list)) #делал по аналогии из примера по обработке списков. К каждому элементу списка применяю метод lower/

# 4) Получить из list пункта 3 dict, ключами которого являются слова, а значениями их количество появлений в тексте.

new_text_list_dict = {new_text_list[a]: new_text_list.count(new_text_list[a]) for a in range(len(new_text_list))} #инициализация словоря с помощью цикла (смотри пример из урока). count считает количество вхождений объекта в списке (или строке?)

# 5) Вывести 5 наиболее часто встречающихся слов (sort), вывести количество разных слов в тексте (set).

text_values = list(new_text_list_dict.values()) #переменная типа list содержит все значения из словоря
